...
class Exercise3Fragment : BaseFragment() {
    ...
    private var showingElapsedTime = false
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        txtElapsedTime.text = "Time: Not started"
    ...
    private fun startShowingElapsedTime(){
        showingElapsedTime = true
    }

    private fun startHidingElapsedTime(){
        showingElapsedTime = false
    }

    private suspend fun updateElapsedTime(startTime: Long){
        logThreadInfo("updateElapsedTime(${startTime})")
        while (showingElapsedTime){
            txtElapsedTime.text = "${(System.currentTimeMillis() - startTime)} Milliseconds"
            delay(1)
        }
    }
